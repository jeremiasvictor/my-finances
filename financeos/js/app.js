...
import {
  addTransaction, getMonthTransactions, instantiateFixedBills,
  togglePaid, deleteTransaction,
  addFixedTemplate, getFixedTemplates, deleteFixedTemplate,
} from "./db.js";
import { getState, setState, subscribe, selectTotals } from "./state.js";
import {
  renderMonthPicker, renderSummary, renderChecklist,
  renderOtherTxs, renderDonutLegend, updateTxCount,
  setLoading, toast, fmt,
} from "./ui.js";
import { renderDonut, renderLiquidityChart } from "./charts.js";
import {
  openTransactionModal, openTemplateModal, openModal, closeModal,
} from "./modal.js";
...
(function wireAuthForm() {
  ...
  const setMode = (m) => {
    mode = m;
    document.getElementById("auth-title").textContent   = m === "login" ? "Entre na sua conta" : "Crie sua conta";
    document.getElementById("auth-submit").textContent  = m === "login" ? "Entrar" : "Criar conta";
...
function render(state) {
  ...
  renderOtherTxs(txs, async (id) => {
    if (!confirm("Excluir este lançamento?")) return;
    await deleteTransaction(user.uid, id);
    toast("Lançamento excluído");
    await loadMonth();
  });
  renderDonut("donut-canvas", fixed, variable);
  renderDonutLegend(fixed, variable);
...
function navMonth(dir) {
  ...
  setState({ month, year });
  renderMonthPicker(month, year, () => navMonth(-1), () => navMonth(1));
  loadMonth();
}
...
onAuth(async (user) => {
  ...
  if (user) {
    ...
    document.getElementById("fab")?.addEventListener("click", () => {
      openTransactionModal(async (payload) => {
        await addTransaction(user.uid, { ...payload, month: getState().month, year: getState().year });
